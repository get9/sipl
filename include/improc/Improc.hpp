                                ...
                                const InterpolateType interpolator)
{
    ...
    auto c1 = homogenize(transform * Vector3d{0, double(image.dims[1]), 1});
    auto c2 = homogenize(transform * Vector3d{double(image.dims[0]), 0, 1});
    auto c3 = homogenize(
        transform * Vector3d{double(image.dims[0]), double(image.dims[1]), 1});
    auto v = Vector3d{double(image.dims[0]), double(image.dims[1]), 1};

    int32_t xmin = std::min({c0[1], c1[1], c2[1], c3[1]});
    int32_t xmax = std::max({c0[1], c1[1], c2[1], c3[1]});
    int32_t ymin = std::min({c0[0], c1[0], c2[0], c3[0]});
    int32_t ymax = std::max({c0[0], c1[0], c2[0], c3[0]});

    // Create new matrix
    Matrix33d inverse = inv(transform);
    MatrixX<T> new_image(xmax - xmin, ymax - ymin);

    // Start filling it
    for (int32_t i = 0; i < new_image.dims[1]; ++i) {
        for (int32_t j = 0; j < new_image.dims[0]; ++j) {
            Vector3d uv{double(i + ymin), double(j + xmin), double(1)};
            Vector3d xy = homogenize(inverse * uv);

            // Skip any points outside image space
            if (xy[0] < 0 || xy[0] >= image.dims[0] || xy[1] < 0 ||
                xy[1] >= image.dims[1]) {
                new_image(j, i) = T(0);
                continue;
            }
            ...
            case InterpolateType::NEAREST_NEIGHBOR:
                new_image(j, i) = image(int32_t(std::round(xy[1])),
            ...
            case InterpolateType::BILINEAR:
                ...
                auto tmp_i1 = ((ceil_col - xy[0]) / (ceil_col - floor_col)) *
                                  image(floor_row, floor_col) +
                              ((xy[0] - floor_col) / (ceil_col - floor_col)) *
                                  image(ceil_row, floor_col);
                auto tmp_i2 = ((ceil_col - xy[0]) / (ceil_col - floor_col)) *
